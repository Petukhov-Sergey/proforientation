...
export default {
    ...
    setup(props) {
        ...
        const openModal = (existingAnswer) => {
            answer.value = {
                content: existingAnswer.content,
                results: existingAnswer.results.map(result => result.id),
                ratings: existingAnswer.results.reduce((acc, result) => {
                    acc[result.id] = result.pivot.rating;
                    return acc;
                }, {})
            };
            modalInstance.show();
        ...
        const saveChanges = () => {
            errors.value = [];
            axios.put(`/editor/questions/${props.questionId}/answers/${answer.value.id}`, {
        ...
        onMounted(() => {
            modalInstance = new Modal(document.getElementById('editAnswerModal'));
        });
